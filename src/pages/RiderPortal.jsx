...
export default function RiderPortal() {
  ...
  const loadDeliveries = () => {
    setLoading(true);
    riderApi.get('/rider/deliveries')
      .then((res) => setOrders(res.data.orders))
      .catch((err) => {
        if (err.response?.status === 401) logout(); // expired token
  ...
  const submitPassword = async (e) => {
    ...
    try {
      const { data } = await riderApi.post('/rider/set-password', { password: pwForm.password });
  ...
  const inputClass = 'w-full px-4 py-3 rounded-lg border border-black/15 bg-white text-sm focus:outline-none focus:border-gold';
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-sm">
          ...
          <p className="mt-2 text-sm text-black/40 text-center">
            New riders: enter the PIN sent to you by SMS. Already set a password? Enter that instead.
          </p>
          <form onSubmit={login} className="mt-8 space-y-3">
            ...
            <label className={labelClass}>PIN or Password *
              <input
                required
                type="password"
            ...
            <p className="text-xs text-black/40 text-center">
              Forgot your password? Ask the shop admin to reset your PIN — you'll receive a new one by SMS.
            </p>
          </form>
        </div>
      </div>
  ...
  if (mustSetPassword) {
    return (
      <div className="flex-1 flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-sm">
          <h1 className="font-display text-4xl text-center">Set Your Password</h1>
          <p className="mt-2 text-sm text-black/40 text-center">
            Welcome, {rider.name}. For security, set a password now — you'll use it to sign in from now on
            instead of the PIN.
          </p>
          <form onSubmit={submitPassword} className="mt-8 space-y-3">
            <label className={labelClass}>New password * (min. 6 characters)
              <input
                required
                type="password"
  ...
  return (
    <div className="w-full mx-auto max-w-2xl px-4 py-12">
      ...
      {justDelivered.map((num) => (
        <p key={num} className="mt-4 text-sm text-green-700 bg-green-50 rounded-lg px-4 py-3 flex items-center gap-2">
      ...
      <h2 className="font-display text-2xl mt-8">
        {loading ? 'Loading deliveries…' : orders.length
      ...
      <div className="mt-4 space-y-4">
        {orders.map((order) => (
          <div key={order.id} className="bg-white border border-black/5 rounded-lg p-5">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <p className="font-mono text-sm">{order.orderNumber}</p>
                <p className="text-xs text-black/40 mt-0.5">Ordered {formatDate(order.createdAt)}</p>
              </div>
            ...
            <div className="mt-3 text-sm">
              <p className="text-gold flex items-center gap-1.5">
                <MapPin size={14} strokeWidth={2} className="shrink-0" /> {order.shippingAddress}
              </p>
              <p className="mt-1 text-black/60">
                {order.User?.firstName} {order.User?.lastName}
                {order.User?.phoneNumber && (
                  <> · <a href={`tel:${order.User.phoneNumber}`} className="text-gold hover:underline">{order.User.phoneNumber}</a></>
                )}
              </p>
            </div>
            <button
            ...
            >
              {confirming === order.id ? 'Confirming…' : <>Confirm Delivery <Check size={16} strokeWidth={2.5} /></>}
            </button>
          </div>
        ))}
      </div>
